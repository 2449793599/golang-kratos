...
func (r *Registry) Register(_ context.Context, si *registry.ServiceInstance) error {
	...
	for _, endpoint := range si.Endpoints { // 遍历服务实例的所有端点：http://192.168.52.1:8000，grpc://192.168.52.1:9000
		...
		if si.Metadata == nil {
			...
			if w, ok := si.Metadata["weight"]; ok { // 重新调整权重
		...
		_, e := r.cli.RegisterInstance(vo.RegisterInstanceParam{ // NACOS服务注册需要的参数
			Ip:          host,
			Port:        uint64(p),
